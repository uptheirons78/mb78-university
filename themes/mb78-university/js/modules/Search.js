			...
			this.resultsDiv.html(`
				<div class="row">
					...
					<div class="one-third">
						...
						${results.generalInfo.length ? '<ul class="link-list min-list">' : '<p>No Matches For Your Search</p>'}
					...
					<div class="one-third">
						...
						${results.programs.length ? '<ul class="link-list min-list">' : '<p>No Matches For Your Search</p>'}
						${results.programs.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join('')}
						${results.programs.length ? '</ul>' : ''}
						
						<h2 class="search-overlay__section-title">Professors</h2>
						${results.professors.length ? '<ul class="professor-cards">' : '<p>No Matches For Your Search</p>'}
					...
					<div class="one-third">
						...
						${results.campuses.length ? '<ul class="link-list min-list">' : '<p>No Matches For Your Search</p>'}
						${results.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join('')}
						${results.campuses.length ? '</ul>' : ''}
						
						<h2 class="search-overlay__section-title">Events</h2>
						${results.events.length ? '' : '<p>No Matches For Your Search</p>'}
						${results.events.map(item => `
							<div class="event-summary">
							  ...
							  <div class="event-summary__content">
							    ...
							    <p>${item.description} <a href="#" class="nu gray">Learn more</a></p>
							  </div>
							</div>
						`).join('')}
					</div>
